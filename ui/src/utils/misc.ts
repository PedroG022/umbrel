...
export function appToUrl(app: UserApp) {
	...
	// For HTTPS mode (e.g. Mode 2 private domain with SSL), route via subdomain
	if (location.protocol === 'https:') {
		return `${location.protocol}//${app.id}.${location.hostname}`
	}
	// For HTTP modes (Mode 1 local mDNS, Mode 3 Tailscale MagicDNS, or direct IP), route via port
